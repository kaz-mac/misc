...
void* malloc_memory(memory_type_t type, size_t size, bool quiet=false) {
  ...
  switch (type) {
    ...
    case type_dma:       ptr = heap_caps_malloc(size, MALLOC_CAP_DMA); break;
    case type_internal:  ptr = heap_caps_malloc(size, MALLOC_CAP_INTERNAL); break;
    case type_spiram:    ptr = heap_caps_malloc(size, MALLOC_CAP_SPIRAM); break;
  }
...
void test_memory_copy_sub(memory_type_t from_type, memory_type_t to_type, int size) {
  if (size < 32) return;
  ...
  if (from_type == type_progmem) {
    ...
    from_memory = (byte*) malloc_memory(from_type, size);
  }
  to_memory = (byte*) malloc_memory(to_type, size);
  sp("  Start memory copy"); delay(100);
  ...
  if (from_memory != NULL && to_memory != NULL) {
    ...
    // コピー元の初期データを作成
    if (from_type != type_progmem) {
      memset(from_memory, 0x00, size);
      // for (int i=0; i<size; i++) from_memory[i] = i % 256; // この方法だとinternalの場合panicで落ちる（アライメントの問題?）
      byte buff[32];
      for (int i=0; i<32; i++) buff[i] = i;
      for (int i=0; i<size; i+=32) {
        size_t copy_size = (i+32 > size) ? size-i : 32;
        memcpy(from_memory+i, buff, copy_size);
      }
    }
    ...
    if (memcmp(from_memory, to_memory, size) == 0) {
      ...
      sp("  ERROR: Data mismatch detected!");
    }
  } else {
  ...
  spf("  From:%s To:%s Size: %d bytes Copy: %s us (%.1f MB/s) %s\n", 
  ...
  for (int s=0; s<array_length(sizes); s++) {
    for (int f=0; f<array_length(from_type); f++) {
      for (int t=0; t<array_length(to_type); t++) {
        ...
        test_memory_copy_sub(from_type[f], to_type[t], sizes[s]);
      }
    }
  }
}
